"""
floe_logger.py - logging functionality for the floe package.
...
class FloeLogger(object):
    """
    The `FloeLogger` class represents the common configuration of the logger
    used in `floe`. By default, this class encapsulates a Python `logging`
    ...
    def write(self, message: str, level: int = None):
        ...
        Args:
            ...
            level: int, default `None`
                ...
                will default to the level of this `FloeLogger` object.
        """
        if level is None:
            level = self.level
...
# that imports this file to the module namespace, e.g. floe_logger.logger
logger = FloeLogger('floe')
